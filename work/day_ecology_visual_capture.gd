...
func _capture() -> void:
	...
	hud.restore_save_data({"money": 20000, "day": 12, "day_progress": 0.76})
	...
	controller.try_place_building(&"dog_house", Vector2(1024, 350))
	controller.try_place_building(&"dog_house", Vector2(600, 700))
	controller.try_place_building(&"shepherd_house", Vector2(760, 700))
	...
	await process_frame
	await process_frame
	root.get_viewport().get_texture().get_image().save_png("res://work/day_ecology_dog_capture.png")
	...
	await process_frame
	await process_frame
	root.get_viewport().get_texture().get_image().save_png("res://work/day_ecology_night_capture.png")
